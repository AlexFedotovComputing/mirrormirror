...
BUNDLE_PROJECTION_PLANE_CENTER_M = (-1.387778781e-17, 1.387778781e-17, -3.4505)
BUNDLE_PROJECTION_PLANE_RADIUS_M = 1.27487
...
PROJECTION_TRAPEZOID_COUNT = 12
PROJECTION_TRAPEZOID_INNER_RADIUS_M = 0.403
PROJECTION_TRAPEZOID_RADIAL_LENGTH_M = 0.800
PROJECTION_TRAPEZOID_INNER_BASE_M = 0.176
PROJECTION_TRAPEZOID_OUTER_BASE_M = 0.606
SELECTED_PRISM_TRAPEZOID_INDEX = PROJECTION_TRAPEZOID_COUNT // 2
SELECTED_PRISM_SECTION_LENGTHS_M = (0.190, 0.200, 0.207, 0.203)
...
def _serve_interactive_plot(
    *,
    outdir: Path,
    backend: str,
    max_interactions: int,
    additional_plot_names: Sequence[str] = (),
) -> None:
    ...
    webbrowser.open(url)
    for plot_name in additional_plot_names:
        additional_url = f"http://{host}:{port}/{plot_name}"
        print(f"Interactive plot: {additional_url}")
        webbrowser.open(additional_url, new=1)
    try:
...
def collect_bundle_ray_projections(
    result: object,
    *,
    plane_center: Sequence[float] = BUNDLE_PROJECTION_PLANE_CENTER_M,
    plane_radius: float = BUNDLE_PROJECTION_PLANE_RADIUS_M,
) -> Dict[int, Dict[str, np.ndarray]]:
    """Orthogonally project post-BUNDLE segments onto a circular XY plane."""
    reflected_parent_groups: Dict[int, int] = {}
    for block in result.segments:
        surfaces = np.asarray(block["surface"], dtype=object)
        ray_ids = to_numpy(block["ray_id"])
        for ray_id, surface_value in zip(ray_ids, surfaces):
            surface = str(surface_value)
            if not surface.startswith("BUNDLE_"):
                continue
            parts = surface.split(None, 1)[0].split("_")
            if len(parts) >= 3 and parts[2].isdigit():
                group = int(parts[2])
                if group in BUNDLE_PROJECTION_COLORS:
                    reflected_parent_groups[int(ray_id)] = group

    center = np.asarray(plane_center, dtype=float)
    collected: Dict[int, Dict[str, List[float]]] = {
        group: {"x0": [], "y0": [], "x1": [], "y1": [], "power": [], "intensity": []}
        for group in BUNDLE_PROJECTION_COLORS
    }
    projected_child_ids: set[int] = set()
    ...
    for block in result.segments:
        ...
        for i, parent_id_value in enumerate(parent_ids):
            group = reflected_parent_groups.get(int(parent_id_value))
            child_id = int(ray_ids[i])
            if group is None or child_id in projected_child_ids:
                continue
            p0 = np.array((float(x0[i]), float(y0[i])), dtype=float)
            p1 = np.array((float(x1[i]), float(y1[i])), dtype=float)
            direction = p1 - p0
            offset = p0 - center[:2]
            ...
            c = float(np.dot(offset, offset)) - float(plane_radius) ** 2
            discriminant = b * b - 4.0 * a * c
            if discriminant < 0.0:
                continue
            root = math.sqrt(max(0.0, discriminant))
            raw_t_enter = (-b - root) / (2.0 * a)
            raw_t_exit = (-b + root) / (2.0 * a)
            t_enter = max(0.0, raw_t_enter)
            t_exit = min(1.0, raw_t_exit)
            if t_exit < t_enter:
                continue
            clipped_p0 = p0 + t_enter * direction
            clipped_p1 = p0 + t_exit * direction
            collected[group]["x0"].append(float(clipped_p0[0]))
            collected[group]["y0"].append(float(clipped_p0[1]))
            collected[group]["x1"].append(float(clipped_p1[0]))
            collected[group]["y1"].append(float(clipped_p1[1]))
            collected[group]["power"].append(float(powers[i]))
            collected[group]["intensity"].append(float(intensities[i]))
            projected_child_ids.add(child_id)

    return {
        group: {key: np.asarray(values, dtype=float) for key, values in data.items()}
        for group, data in collected.items()
    }


def build_projection_trapezoids(
    *,
    center: Sequence[float] = BUNDLE_PROJECTION_PLANE_CENTER_M,
) -> List[np.ndarray]:
    center_xy = np.asarray(center[:2], dtype=float)
    outer_radius = PROJECTION_TRAPEZOID_INNER_RADIUS_M + PROJECTION_TRAPEZOID_RADIAL_LENGTH_M
    trapezoids: List[np.ndarray] = []
    for index in range(PROJECTION_TRAPEZOID_COUNT):
        angle = 0.5 * math.pi + index * 2.0 * math.pi / PROJECTION_TRAPEZOID_COUNT
        radial = np.array((math.cos(angle), math.sin(angle)), dtype=float)
        tangent = np.array((-math.sin(angle), math.cos(angle)), dtype=float)
        inner_center = center_xy + PROJECTION_TRAPEZOID_INNER_RADIUS_M * radial
        outer_center = center_xy + outer_radius * radial
        points = np.vstack(
            (
                inner_center - 0.5 * PROJECTION_TRAPEZOID_INNER_BASE_M * tangent,
                outer_center - 0.5 * PROJECTION_TRAPEZOID_OUTER_BASE_M * tangent,
                outer_center + 0.5 * PROJECTION_TRAPEZOID_OUTER_BASE_M * tangent,
                inner_center + 0.5 * PROJECTION_TRAPEZOID_INNER_BASE_M * tangent,
            )
        )
        trapezoids.append(np.vstack((points, points[0])))
    return trapezoids
...
def clip_segment_to_convex_polygon(
    ...
    """Clip a 2D segment to a counter-clockwise convex polygon."""
    ...
    for index, edge_start in enumerate(vertices):
        ...
        relative = p0 - edge_start
        c0 = float(edge[0] * relative[1] - edge[1] * relative[0])
        cd = float(edge[0] * direction[1] - edge[1] * direction[0])
        if abs(cd) <= 1e-15:
            if c0 < -1e-12:
                return None
            continue
...
def build_selected_prism_sections(prism: np.ndarray | None = None) -> Dict[str, np.ndarray]:
    if prism is None:
        prism = build_projection_trapezoids()[SELECTED_PRISM_TRAPEZOID_INDEX]
    vertices = np.asarray(prism, dtype=float)[:4]
    inner_left, outer_left, outer_right, inner_right = vertices
    sections: Dict[str, np.ndarray] = {}
    distance_start = 0.0
    for index, section_length in enumerate(SELECTED_PRISM_SECTION_LENGTHS_M, start=1):
        distance_end = distance_start + section_length
        start_fraction = distance_start / PROJECTION_TRAPEZOID_RADIAL_LENGTH_M
        end_fraction = distance_end / PROJECTION_TRAPEZOID_RADIAL_LENGTH_M
        left_start = inner_left + start_fraction * (outer_left - inner_left)
        left_end = inner_left + end_fraction * (outer_left - inner_left)
        right_start = inner_right + start_fraction * (outer_right - inner_right)
        right_end = inner_right + end_fraction * (outer_right - inner_right)
        points = np.vstack((left_start, left_end, right_end, right_start))
        sections[f"A{index}"] = np.vstack((points, points[0]))
        distance_start = distance_end
    return sections


def write_bundle_projection_plot(path: Path, result: object) -> Dict[int, int]:
    import plotly.graph_objects as go

    projections = collect_bundle_ray_projections(result)
    center_x, center_y, center_z = BUNDLE_PROJECTION_PLANE_CENTER_M
    theta = np.linspace(0.0, 2.0 * math.pi, 361)
    circle_x = center_x + BUNDLE_PROJECTION_PLANE_RADIUS_M * np.cos(theta)
    circle_y = center_y + BUNDLE_PROJECTION_PLANE_RADIUS_M * np.sin(theta)
    ...
    fig.add_scatter(
        ...
        mode="lines",
        fill="toself",
        fillcolor="rgba(226, 232, 240, 0.28)",
    ...
    counts: Dict[int, int] = {}
    for group, color in BUNDLE_PROJECTION_COLORS.items():
        data = projections[group]
        counts[group] = int(data["x0"].size)
        line_x: List[float | None] = []
        line_y: List[float | None] = []
        for x_start, y_start, x_end, y_end in zip(data["x0"], data["y0"], data["x1"], data["y1"]):
            line_x.extend((float(x_start), float(x_end), None))
            line_y.extend((float(y_start), float(y_end), None))
        fig.add_scatter(
            x=line_x,
            y=line_y,
            mode="lines",
            name=f"{group} лазерная плоскость",
            line={"width": 1.5, "color": color},
            opacity=0.62,
            hovertemplate=f"{group} лазерная плоскость<br>x=%{{x:.7f}} м<br>y=%{{y:.7f}} м<extra></extra>",
        )

    trapezoid_x: List[float | None] = []
    trapezoid_y: List[float | None] = []
    for trapezoid in build_projection_trapezoids():
        trapezoid_x.extend([*(float(value) for value in trapezoid[:, 0]), None])
        trapezoid_y.extend([*(float(value) for value in trapezoid[:, 1]), None])
    fig.add_scatter(
        x=trapezoid_x,
        y=trapezoid_y,
        mode="lines",
    ...
    fig.update_layout(
        title=(
            "Orthogonal projections of rays reflected from BUNDLE mirrors"
            f"<br><sup>center=({center_x:.10g}, {center_y:.10g}, {center_z:.6g}) m, "
            f"radius={BUNDLE_PROJECTION_PLANE_RADIUS_M:.5f} m</sup>"
        ),
        xaxis={
            "title": "x [m]",
            "range": [center_x - BUNDLE_PROJECTION_PLANE_RADIUS_M, center_x + BUNDLE_PROJECTION_PLANE_RADIUS_M],
            "scaleanchor": "y",
            "scaleratio": 1.0,
            "constrain": "domain",
            "showgrid": True,
        ...
        yaxis={
            "title": "y [m]",
            "range": [center_y - BUNDLE_PROJECTION_PLANE_RADIUS_M, center_y + BUNDLE_PROJECTION_PLANE_RADIUS_M],
            "showgrid": True,
            "zeroline": True,
        },
        margin={"l": 75, "r": 260, "t": 125, "b": 70},
        font={"size": 16},
        legend={
            "orientation": "v",
            "yanchor": "top",
        ...
        annotations=[
            {
                "text": (
                    "Чтобы рассмотреть одну лазерную<br>"
                    "плоскость, дважды щёлкните по её<br>"
                    "названию в легенде"
                ),
                "xref": "paper",
                "yref": "paper",
...
def write_selected_prism_projection_plot(path: Path, result: object) -> Dict[int, int]:
    import plotly.graph_objects as go

    projections = collect_bundle_ray_projections(result)
    prism = build_projection_trapezoids()[SELECTED_PRISM_TRAPEZOID_INDEX]
    sections = build_selected_prism_sections(prism)
    fig = go.Figure()
    whole_trace_indices: List[int] = []
    whole_ray_trace_indices: List[int] = []
    fig.add_scatter(
        x=prism[:, 0],
        y=prism[:, 1],
        mode="lines",
    ...
    whole_trace_indices.append(len(fig.data) - 1)
    ...
    for group, color in BUNDLE_PROJECTION_COLORS.items():
        ...
        for x_start, y_start, x_end, y_end in zip(data["x0"], data["y0"], data["x1"], data["y1"]):
            ...
            line_x.extend((float(clipped_start[0]), float(clipped_end[0]), None))
            line_y.extend((float(clipped_start[1]), float(clipped_end[1]), None))
            count += 1
        counts[group] = count
        fig.add_scatter(
            x=line_x,
        ...
        whole_trace_indices.append(len(fig.data) - 1)
        whole_ray_trace_indices.append(len(fig.data) - 1)

    divider_x: List[float | None] = []
    divider_y: List[float | None] = []
    label_x: List[float] = []
    label_y: List[float] = []
    section_labels: List[str] = []
    for section_name, section in sections.items():
        outer_edge = section[1:3]
        if section_name != "A4":
            divider_x.extend((float(outer_edge[0, 0]), float(outer_edge[1, 0]), None))
            divider_y.extend((float(outer_edge[0, 1]), float(outer_edge[1, 1]), None))
        label_x.append(float(np.mean(section[:4, 0])))
        label_y.append(float(np.mean(section[:4, 1])))
    ...
    # Draw the outline again so it remains visible above the ray projections.
    fig.add_scatter(
        x=prism[:, 0],
        y=prism[:, 1],
        mode="lines",
    ...
    section_trace_indices: Dict[str, List[int]] = {}
    section_ray_trace_indices: Dict[str, List[int]] = {}
    for section_name, section in sections.items():
        section_trace_indices[section_name] = []
        section_ray_trace_indices[section_name] = []
        for group, color in BUNDLE_PROJECTION_COLORS.items():
            data = projections[group]
            ...
            for x_start, y_start, x_end, y_end in zip(data["x0"], data["y0"], data["x1"], data["y1"]):
                ...
                if clipped is None:
                    continue
                clipped_start, clipped_end = clipped
                section_x.extend((float(clipped_start[0]), float(clipped_end[0]), None))
                section_y.extend((float(clipped_start[1]), float(clipped_end[1]), None))
            fig.add_scatter(
                x=section_x,
                y=section_y,
                mode="lines",
                ...
                opacity=0.82,
                hovertemplate=(
                    f"{group} лазерная плоскость · {section_name}"
                    "<br>x=%{x:.7f} м<br>y=%{y:.7f} м<extra></extra>"
                ),
                visible=False,
                showlegend=False,
            )
            trace_index = len(fig.data) - 1
            section_trace_indices[section_name].append(trace_index)
            section_ray_trace_indices[section_name].append(trace_index)
        fig.add_scatter(
            x=section[:, 0],
            y=section[:, 1],
            mode="lines",
        ...
        section_trace_indices[section_name].append(len(fig.data) - 1)
        fig.add_scatter(
            x=[float(np.mean(section[:4, 0]))],
            y=[float(np.mean(section[:4, 1]))],
            mode="text",
            text=[section_name],
    ...
    padding = 0.06
    center_x, center_y, center_z = BUNDLE_PROJECTION_PLANE_CENTER_M
    whole_x_range = [float(np.min(prism[:, 0])) - padding, float(np.max(prism[:, 0])) + padding]
    whole_y_range = [float(np.min(prism[:, 1])) - padding, float(np.max(prism[:, 1])) + padding]
    section_buttons: List[Dict[str, object]] = []
    for section_name, section in sections.items():
        section_padding = 0.025
        visible = [False] * len(fig.data)
        showlegend = [False] * len(fig.data)
        for trace_index in section_trace_indices[section_name]:
            visible[trace_index] = True
        for trace_index in section_ray_trace_indices[section_name]:
        ...
        section_buttons.append(
            {
                ...
                "args": [
                    ...
                    {
                        "xaxis.range": [
                            float(np.min(section[:, 0])) - section_padding,
                            float(np.max(section[:, 0])) + section_padding,
                        ],
                        "yaxis.range": [
                            float(np.min(section[:, 1])) - section_padding,
                            float(np.max(section[:, 1])) + section_padding,
                        ],
                    }
                ],
            }
    ...
    section_buttons.append(
        {
            ...
            "args": [
                {
                    "visible": [index in whole_trace_indices for index in range(len(fig.data))],
                    "showlegend": [index in whole_ray_trace_indices for index in range(len(fig.data))],
                },
                {"xaxis.range": whole_x_range, "yaxis.range": whole_y_range},
            ],
        }
    )
    fig.update_layout(
        title=(
            "Проекции лучей на выделенную призму"
            f"<br><sup>плоскость z={center_z:.6g} м; центр плоскости "
            f"({center_x:.10g}, {center_y:.10g}) м</sup>"
        ),
        xaxis={
            "title": "x [м]",
            "range": whole_x_range,
...
def main() -> None:
    ...
    if args.plot:
        ...
        bundle_projection_path = outdir / "bundle_ray_projections.html"
        selected_prism_projection_path = outdir / "selected_prism_ray_projections.html"
        bundle_1_1 = make_bundle_1_1()
        bundle_1_2 = make_bundle_1_2()
        bundle_1_3 = make_bundle_1_3()
        bundle_1_4 = make_bundle_1_4()
        ...
        projection_counts = write_bundle_projection_plot(bundle_projection_path, result)
        prism_projection_counts = write_selected_prism_projection_plot(selected_prism_projection_path, result)
        print(f"Wrote: {plot_path}")
        print(f"Wrote: {bundle_projection_path}")
        print(f"Wrote: {selected_prism_projection_path}")
        print(
            "Bundle ray projections: "
            + ", ".join(f"Bundle_*_{group}={count}" for group, count in projection_counts.items())
        )
        print(
            "Selected prism ray projections: "
            + ", ".join(f"Bundle_*_{group}={count}" for group, count in prism_projection_counts.items())
        )
        if args.open_plot:
            _serve_interactive_plot(
                ...
                backend=args.backend,
                max_interactions=args.max_interactions,
                additional_plot_names=(bundle_projection_path.name, selected_prism_projection_path.name),
            )
